            ...
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws_creds', accessKeyVariable: 'TF_VAR_access_key', secretKeyVariable: 'TF_VAR_secret_key']]){
                    withCredentials([string(credentialsId: 'git-token', variable: 'TF_VAR_git_token')]) {
                        withCredentials([sshUserPrivateKey(credentialsId: 'ssh_instance', keyFileVariable: 'TF_VAR_private_key', usernameVariable: 'TF_VAR_ssh_user')]) {
        ...
        stage('Terraform apply') {
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'AWS_keys', accessKeyVariable: 'TF_VAR_access_key', secretKeyVariable: 'TF_VAR_secret_key']]){
                    withCredentials([string(credentialsId: 'git-token', variable: 'TF_VAR_git_token')]) {
                        withCredentials([sshUserPrivateKey(credentialsId: 'ssh_instance', keyFileVariable: 'TF_VAR_private_key', usernameVariable: 'TF_VAR_ssh_user')]) {
                            sh 'cd ./jenkins_terraform_final; terraform apply -auto-approve'
                        }
                    }
                }
            }
